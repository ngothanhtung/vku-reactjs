...
export default function TaskFilterForm({ onFilterChange, onClearFilters, initialFilters }: Props) {
  const {
    ...
    defaultValues: {
      ...
      dueDateFrom: initialFilters?.dueDateFrom?.toISOString().split('T')[0] || '2024-01-01',
      dueDateTo: initialFilters?.dueDateTo?.toISOString().split('T')[0] || '2025-12-01',
      assigneeId: initialFilters?.assigneeId || '',
    },
  });

  // Watch for form changes to apply filters in real-time
  ...
  React.useEffect(() => {
    const subscription = watch((value) => {
      ...
      if (value.status && value.status !== '') {
        filters.status = value.status as Task['status'];
      }

      if (value.priority && value.priority !== '') {
        filters.priority = value.priority as Task['priority'];
      }

      if (value.dueDateFrom) {
        filters.dueDateFrom = new Date(value.dueDateFrom);
      }

      if (value.dueDateTo) {
        filters.dueDateTo = new Date(value.dueDateTo);
      }

      if (value.assigneeId && value.assigneeId.trim() !== '') {
        filters.assigneeId = value.assigneeId.trim();
      }
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {hasActiveFilters && (
          <button
            ...
            className="text-sm bg-gray-100 text-blue-600 hover:text-blue-800 hover:bg-gray-200"
          >
            Clear All Filters
          </button>
        )}
