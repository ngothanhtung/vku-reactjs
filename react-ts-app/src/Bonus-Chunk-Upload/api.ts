...
const API = {
  init: async (filename: string, totalChunks: number, totalSize: number, fileHash?: string): Promise<InitResponse> => {
    ...
    const r = await fetch(`${BASE_URL}/upload/init`, {
      method: 'POST',
      headers: {
  ...
  status: async (uploadId: string): Promise<StatusResponse> => {
    const r = await fetch(`${BASE_URL}/upload/status?uploadId=${encodeURIComponent(uploadId)}`);
  ...
  uploadChunk: async (params: UploadChunkParams): Promise<UploadChunkResponse> => {
    ...
    while (true) {
      try {
        const r = await fetch(`${BASE_URL}/upload/chunk`, { method: 'POST', body: form });
  ...
  complete: async (uploadId: string, filename: string, fileHash: string): Promise<CompleteResponse> => {
    const body = {
      uploadId,
      filename,
      fileHash,
    ...
    const r = await fetch(`${BASE_URL}/upload/complete`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
